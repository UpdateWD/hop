...
async def start_msg(message: types.Message):
	if message.chat.type == 'private':
		...
		await message.answer_photo(photo=photo, caption=f'<b>Welcome {message.from_user.get_mention()}. GG Market - Best Market</b>', reply_markup=await main_menu(user_id))

@vip.callback_query_handler(text='back', state='*')
async def back_msg(call: types.CallbackQuery):
	await call.message.edit_caption(caption=f'<b>Welcome {call.from_user.get_mention()}. GG Market - Best Market</b>', reply_markup=await main_menu(call.from_user.id))
...
async def check_crypto_bot_funds(call: types.CallbackQuery):
	try:
		if await check_crypto_bot_invoice(int(call.data.split('|')[1])):
			...
			await call.message.edit_caption(caption=f'<b>Welcome {call.from_user.get_mention()}. FTM Market - Best Market</b>', reply_markup=await main_menu(call.from_user.id))
